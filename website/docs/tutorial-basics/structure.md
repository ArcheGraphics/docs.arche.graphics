...
标准库的`std::shared_ptr` 和 `std::unique_ptr`, 尽管直接用标准库的引用计数类未必是一个最佳实践，但免除实现垃圾回收机制可以使得代码中减少很多宏的处理，简化引擎的代码。对于类似 `Entity`
这样唯一的实体，我使用 `std::unique_ptr`，对于公共的资源类型，例如 `Mesh` 和 `Material` 我采用了 `std::shared_ptr`。
